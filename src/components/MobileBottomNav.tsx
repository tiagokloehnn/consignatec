...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <nav
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/90 px-2 py-1 pb-safe shadow-[0_-4px_16px_rgba(0,0,0,0.06)]"
    >
      <div className="flex items-center justify-around max-w-md mx-auto">
        ...
        >
          ...
          {expensesCount > 0 && (
            <span className="absolute top-1 right-2 h-4 w-4 rounded-full bg-teal-800 text-white text-[9px] font-bold flex items-center justify-center">
        ...
        <div className="relative -top-3">
          <button
            ...
            className="h-12 w-12 rounded-full bg-gradient-to-tr from-teal-800 to-teal-600 text-white flex items-center justify-center shadow-lg shadow-teal-900/30 hover:scale-105 active:scale-95 transition-transform border-2 border-white focus:outline-none"
        ...
        <button
          ...
          className="flex flex-col items-center justify-center py-1 px-2 min-w-[56px] min-h-[44px] text-slate-500 hover:text-teal-800 transition-colors rounded-lg"
        >
          <Sparkles className="h-5 w-5 text-emerald-600 animate-pulse" />
          <span className="text-[10px] mt-0.5 text-slate-700">IA</span>
        </button>
      </div>
    </nav>
  );
